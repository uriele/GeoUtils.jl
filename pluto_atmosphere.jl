...
# ╔═╡ 53bcfec0-a689-11ef-269c-77e53c95a246
begin
	using PlutoUI
	using GeoUtils
	...
	using PlutoExtras
	using PlutoExtras
	TableOfContents(title="Atmosphere")
	initialize_eqref()
end

# ╔═╡ dd39d13c-a0c3-4b69-b674-bb8c2ebe97e0
md"""
# Optical Refractive Index

Spectroscopic and metrological measurments are always made in air if possible. Thus, accurate work requires conversion from the waveleght of air to the corresponding wavelength in vacuum

$(texeq"λ_{vac}=nλ_{air} \label{uno}")

In order to make this conversion with maximum accuracy, the value of the refractive index need to be known at least as well as the precision to which the present wavelength standard is defined, 1× 10⁻⁸.
"""

# ╔═╡ ace5b687-563f-44c2-8bee-c4a8d4003b75
md"""
 In practice it is difficult to control the composition of the air in an optical instrument sufficiently well to  achieve this accuracy, and recommended metrological practice requires the actual measurement of the refractive index of the air in the optical path.

 Accurate formulas are still required to evaluate the refractivity (n-1) of air as a function of:
- pressure
- temperature
- humidity
- CO2ppm
...
$(texeq" \frac{n^2-1}{n^2+2}=\sum_i{R_i\rho_i} \label{Lorentz}")

where
- Rᵢ is the specific refraction
- ρᵢ is the partial density of the i-th element of the mixture

The specific refraction, that is invariant under changes in density to a high defree of approximation for the components of air at atmospheric temperature, is given by:
"""

# ╔═╡ d2600870-5dbc-4f4a-8551-8f8524b3908b
texeq"
R_i= \frac{n_i^2-1}{n_i^2+2}\frac{1}{\rho_i}=\frac{4}{3}\pi\frac{N_a}{M_i}Z_i
	\label{ithspecificrefraction}
	"

# ╔═╡ 5af82cc1-2ab2-46ba-b171-2142f0fa9500
md"""
where
- nᵢ is the specific refraction
- Mᵢ is the molecular weight in g/mol
- Nₐ is the the Avocadro number
- Zᵢ is the compressibility

The use of Eq.$(eqref("Lorentz")) to represent the refractive index, assumes that **(a) Lorentz local field is correct** and **(b) the polarizability of the component is unchanged in the mixture**.

Lorentz field only gives the **volume average** of the local field and not the time-averaged field of the individual particles, and such local field is different for different kinds of molecules in the mixture.

It might be shown that the specific refraction of dry air is not completely invariant but increases by about 0.01% for and increase of 0.05atm. This result has only be verified qualitatively (in the 80s) and because it is based on a cavity definition of the local field it may not fully compensate for the known deficiencies of the Lorentz formula. Thus, until absolute measurement of the refrativity as a function of the density are provided, it is better to keep using the Lorentz formula even if we are aware of its shortcomings.

For air, we can write the composition ar a combination of 3 elements:
- Dry-CO2free air (a mixture of oxygen, nitrogen, and argon): since the dispersion curves of the individual components is sufficiently similar.
- Water Vapor
- CO2

## Dry CO₂ free Air

Standard Air is defined to be
- 0% humidity
- 15°C
- 1013.25hPa
-  N₂: 78.09%
...
The refractivity of CO₂ free air can be find from Edlen's (1996)[^1] by multiplying the dispersion formula from standard air by 1.000162 to remove the effect of carbon dioxide, giving:

$(texeq"(n-1)_{air}\times 10^{8}=8340.78+\left[\frac{2405640}{130-\sigma^2}\right]+\left[\frac{15994}{38.9-\sigma^2}\right] \label{ndry}")

[^1] B. Edlen, Metrologia 2,71 (1966).

"""

# ╔═╡ 166f8998-f3e3-49e1-aad0-9bbb8ae63604
md"""

where σ=1/λ is the vacuum wavenumber. The error introduced by adding the multiplicative number to convert into CO2-free air is only over 3×10^{-10} over the range 3650-6328Å

The data used for the derivation of the new density expression were the tabulated values of compressibility factor

$(texeq"Z=\frac{PV}{RT}")

from which the density can be rewritten as:

$(texeq"\rho=\frac{MP}{RT}\frac{1}{Z}")
...
The inverse compressibility factor was given by Hilsenrath [^2]

$(texeq"\frac{1}{Z}=1+P\beta(P)\gamma(T)")

By using a simple quadratic fitting for γ(T)=a+bT+cT² is is possible to obtain with good accuracy β(P)=1. Thus the expression for CO2-free density of dry air can be given as

[^2] J. Hilsenrath et al., Tables of Thermal Properties of Gases, NBS Circular 564 (November 1955).
"""

# ╔═╡ 5dcd6f13-9a7b-4e71-a58c-7b4f0052a140
texeq"\rho(g/m^3)=348.328 \frac{P}{T}\left[1+P\left(
57.90\times 10^{-8}-\frac{0.94581\times 10^{-3}}{T}+\frac{0.25844}{T^{-2}}
\right)\right] \label{rhodry}"
...
Thus, the refractivity Rᵢ of dry air can be found by substituting in Eq. $(eqref("ithspecificrefraction")) the value of the refractive index found in Eq. $(eqref("ndry")) and the density from Eq. $(eqref("rhodry")).

"""

# ╔═╡ bbe7c8cf-00b6-45f3-b371-e165cd7ee6a7
md"""
## Water Vapor
...
Since the compressibility factor of water vapor is not tabulated, the value is obtained using the equation of state of Goff and Gratch[^3]

$(texeq"PV=RT-A_{ww}P-A_{www}P^2 \label{vaporPV}")

where the virial coefficients A_ww and A_www are given by the empirical relationship:

$(texeq"A_{ww}=-0.0302+88.514τ10^{233280\tau^2}")
$(texeq"A_{www}=67.4τ^2A_{ww}^3")

where τ=1/t and t is the temperature in [Rankine]

Thus, the compressibility can be expressed by dividing $(eqref("vaporPV")) by RT:

$(texeq"Z=1-13.7066\cdot A_{ww}\frac{P}{T}-2.90063E4\cdot A_{www}\frac{P^2}{T}")

At a given temperature, only water vapor pressures less than or equal to the saturation pressure Pₛ(T) were used in the evaluation of Z. Which value is tabulated.

[^3] J. A. Goff, in Humidity and Moisture, A. Wexler, Ed. (Rein
hold Publishing Corporation, New York, 1965), Vol. 3, p. 289.
"""

# ╔═╡ 71b4d686-01a5-4ceb-a72c-ccd754f9e3ad
md"""
## Group Refractive Index

Many electronic distance-measuring instruments use intensity-modulated or pulsed light, and therefore require ta knowledge of the group refractive index, rather than the phase index. The group refractive index is found for any component under standard conditions by replacing the correspondin standard phase index (n) by

$(texeq"n_g=n+\sigma\frac{dn}{d\sigma}")
...
The range of validity, even if wider than Owens (and Edlen) is still limited to frequencies far away from the IR, that is the range of interest used by MIPAS, In particular

The density equation is valid in the range
- Temperature -40C to +100C
- Pressure 80 to 120 kPa
- Humidity 0 to 100%
- Wavelength 300nm to 1690nm (in the visible)

Thus it does not work efficiently in the IR (2.5-50μm)

# Mathar Approximation

Mathar [^6] produces different fitting with for different frequencies in the infrared, making it better fitted for the purpose. Moreover, since the contribution of CO2 is negligible around 10μm, the paremeterization of the refraction index is given only as a function of P,T, and humidity


[^5] Philip E. Ciddor, "Refractive index of air: new equations for the visible and near infrared," Appl. Opt. 35, 1566-1573 (1996)

[^6] Richard J. Mathar, "Refractive Index of Humid Air in the Infrared: Model Fits" Journal of Optics A: Pure and Applied Optics Vol 9-5 (2007)
"""

# ╔═╡ 00000000-0000-0000-0000-000000000001
PLUTO_PROJECT_TOML_CONTENTS = """
[deps]
GeoUtils = "9e7f6a50-18ad-4244-91e0-d4fb5b087add"
...
manifest_format = "2.0"
project_hash = "bbbd6974c92fd3e5a6fecff9337c1853308df627"
...
git-tree-sha1 = "96e9c8544d70af2ddc5c5846472689bd05d1469e"
...
[[deps.GeoUtils]]
deps = ["Aqua", "CoordRefSystems", "DataFrames", "GADM", "GLMakie", "GeometryOps", "ImageFiltering", "Interpolations", "PlutoUI", "Reexport", "ScatteredInterpolation", "Unitful", "WGLMakie", "XLSX"]
git-tree-sha1 = "6371413e27ca3d71d854081c35fe662fc3decffd"
uuid = "9e7f6a50-18ad-4244-91e0-d4fb5b087add"
version = "0.3.1"

    [deps.GeoUtils.extensions]
    GeoMakieExt = ["Makie", "GeoMakie"]

    [deps.GeoUtils.weakdeps]
