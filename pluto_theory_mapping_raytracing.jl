...
# ╔═╡ c6c51352-a8c6-11ef-1f49-fd10297cc3cb
begin
...
# ╔═╡ 78b3f9b0-7232-43b9-af55-21dd1f08d6e0
md"""
# Linear Mappings of Ellipses

For ray tracing in 2D on a geoid, it can be useful to work using unitary circles and linear mapping. It is important to notice, however, that since linear mapping does preserve the angles, the normal of the ellipse will not be normal in the unit space.

We want to remember that in the real space the normal and the tangent to a point on the ellipse is defined by:
...
```math
\mathbf{n}(\mathbf{r},t)=\mathbf{r}+t*\hat{n}(\mathbf{r})
\quad
\mathbf{t}(\mathbf{r},t)=\mathbf{r}+t*\hat{t}(\mathbf{t})
```

Since the mapping between the real space and the unit circle is given by a linear scaling:

```math
M_{C\rightarrow E}=\begin{bmatrix}
a && 0 \\
0 && b
\end{bmatrix}
\qquad
M_{E\rightarrow C}=M_{C\rightarrow E}^{-1}=\begin{bmatrix}
1/a && 0 \\
0 && 1/b
\end{bmatrix}
```

Thus, the normal to a point on the ellipse in the ellipse space can also be expressed as:

```math
\hat{n}(x,y)=M_{E\rightarrow C}
\begin{bmatrix}
x \\ y
\end{bmatrix}
\quad
...
manifest_format = "2.0"
project_hash = "72c876f5b87298592296adeedd2558221ed11f6e"
