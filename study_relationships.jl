...
# ╔═╡ bed6f8f0-acf1-11ef-0b02-d1ae91697d54
begin
	using LinearAlgebra
...
=#
	...
	struct AtmosphereMesh{T<:Number}
		x_scale_top::T
		y_scale_top::T
		x_scale_bottom::T
		y_scale_bottom::T
	...
	scale_top=[T(1).-h.*(inva,invb) for h in h]
	scale_bottom=scale_top[1:end-1]
...
# ╔═╡ c735bbc7-c66b-4e76-ab16-c020976046ee
let
	N=360
	a1=b
	b1=majoraxis_earth
	...
	for h in 0:10
		...
		@ begin
			...
			for i in 2:length(circ)
				cc+= circ[i].-circ[i-1] |> x-> sqrt(sum(x->x^2,x))
				cc1+= circb[i].-circb[i-1] |> x-> sqrt(sum(x->x^2,x))
			end
			(cc/2/pi,cc1/2/pi)
		end
		@ majoraxis_earth,b,
...
=#
	#=
	function project!(r::Ray2D,d::AtmosphereElement)
		h1=hit(r,d.top)
		h2=hit(r,d.bottom)
		h3=hit(r,d.left)
		h4=hit(r,d.righ)
...
# ╔═╡ d91e9efd-accc-4e6e-a1fb-ba8a1f38ce8f
let
...
# I want to project it into a space where the lower circ is unitary and the top one projected on the xy plane has radius 2
	...
	ax1.title="Normal Space"
	lines!(ax1,map(θ->_circ(θ;h=h),θ))
	ax2=Axis3(fig[2:3,1])
...
let
	frac(h,b)=1/(1+h/b)
	invfrac(h,h1,b)=frac(h1,b)/frac(h,b)
	figure1=Figure()
...
manifest_format = "2.0"
project_hash = "e02208949ddee08ba1a92cd027a18847f659e4f0"
