...
# ╔═╡ 59d332ee-e498-11ef-120b-c51c7aaf62f9
using Makie,WGLMakie,LinearAlgebra, PlutoExtras

# ╔═╡ 5772041a-97e7-4bbe-909d-b4d183e94430
begin
	...
	@variables t,θ,E,AE,Tₑ,Dᵣ,AETₑ,DᵣTₑ,Tₑ²,Dᵣ²,AEDᵣ
end

# ╔═╡ 4a7f695b-da8d-4201-afcc-37ddc0646ba1
using StaticArrays

# ╔═╡ 82c5a61d-5268-4f02-8813-188b909e29cb


# ╔═╡ aff487be-ca98-4193-aa16-021b7fdf47b9
initialize_eqref()

# ╔═╡ d51ebb1d-089b-4d84-bce6-771c1f02c6b4


# ╔═╡ f383cdc0-e736-48e4-a2cc-956e748164b0


# ╔═╡ 74d6c6fe-0575-4a25-86b1-c88103a671a1


# ╔═╡ 7f931e78-9623-4e45-996d-ad77719f3e2a
ExtendedTableOfContents()

# ╔═╡ 46feb5e6-2bdc-4944-8e3d-2c5f8f33c323


# ╔═╡ a2ca8a73-534c-469c-adf0-dd040dfad7df


# ╔═╡ 47c63e51-8bde-4af3-ae77-7fd7ec405cab


# ╔═╡ 56688992-eb9c-4f7c-b329-ed9312d6a071


# ╔═╡ 3edc0911-6aa7-4f31-aabc-1d4070107a2f


# ╔═╡ b9516ddd-e37b-4bdb-aa30-55c79d7fa5ef


# ╔═╡ 86732b1c-669b-43a7-a9a0-541853d726e7


# ╔═╡ 0510c74b-1439-482c-bc93-90764a379e15


# ╔═╡ 0dbecd02-ac11-40b0-997a-c6689047bef2
...
I can solve the problem by transforming it into an augmented lagrangian problem [1] or by setting one of the derivatives to zero when it gets out of bounds [2] $(eqref("interactive"))
"""

# ╔═╡ a5264286-35c9-43c8-9c44-b448f968ec07
begin
	
	struct Ray{T<:Real}
		origin::SVector{2,T}
		direction::SVector{2,T}
	end
	struct Ellipse{T<:Real}
		b::T
	end
	(r::Ray{T})(t::T) where T= r.origin+t.*r.direction
	(e::Ellipse{T})(t::T) where T=SVector{2,T}(cos(t),e.b*sin(t))
	fun(x,y,r::Ray{T},e::Ellipse{T},h) where T=dot((r(x)-e(y)),(r(x)-e(y)))-h^2
end

# ╔═╡ 592b0dea-4368-44d2-a993-9381e33679d8
r=Ray(SVector(1.,1.),SVector(0.,1.))

# ╔═╡ 3a4a1ef3-0221-4b50-85b3-f634c0a36ff1
r(1.0)

# ╔═╡ 9946416b-3e38-48e7-ac6d-e8f11fd297dd
@bind values PlutoUI.combine() do Child
	md"""
	barrier $(Child(PlutoUI.Slider(0:10))
	)
	distance $(Child(PlutoUI.Slider(0:0.01:2))
	) 
	"""
end

# ╔═╡ 9654517c-f685-47f6-a10f-400e997aa85a
let
	fig=Figure()
	r=Ray(SVector(1.,1.),SVector(-cos(45),-sin(45)))
	el=Ellipse(0.9)
	t=LinRange(0,3,1000)
	θ=LinRange(0,2pi,1000)
	μ=values[1]
	h=values[2]
	ax=Axis(fig[1,1],xlabel="t",ylabel="θ")
	contourf!(ax,t,rad2deg.(θ),[fun(t,θ,r,el,h)+μ*(θ-deg2rad(60))^2 for t in t, θ in θ],levels=LinRange(-3,3,30))
	contour!(ax,t,rad2deg.(θ),[fun(t,θ,r,el,h)+μ*(θ-deg2rad(60))^2 for t in t, θ in θ],labels=true,levels=LinRange(-3,3,30),color=:black)
	fig
...
manifest_format = "2.0"
project_hash = "f25bfc050ce2d7fa2bab24cb9bc92ce1997a90da"
