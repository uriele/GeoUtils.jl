...
rays=map(x-> StaticRay2D(SVec2(x[1],x[2]),SVec2(x[3],x[4])),eachcol(rcoord))
...
SUITE["distance"]["segment"]=@benchmarkable map!(r->distance_from_segment(r,SVec2(0.0,0.0),SVec2(1.0,1.0)),$sdist,$rays)
...
SUITE["distance_distributed"]["segment"]=@benchmarkable pmap(r->distance_from_segment(r,SVec2(0.0,0.0),SVec2(1.0,1.0)),$rays)
...
SUITE["distance_fused"]["segment"]=@benchmarkable sdist .= @. distance_from_segment($rays,SVec2(0.0,0.0),SVec2(1.0,1.0))
#tune!(SUITE)
