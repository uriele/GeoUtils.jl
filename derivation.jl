...
# ╔═╡ 427143b0-ad9b-11ef-378e-ed19e003bce2
using Symbolics

# ╔═╡ 4320bda9-8d4a-4f4f-abda-2292b9a14daa
let
	using LinearAlgebra
	phi=45
	n11=1
...
# ╔═╡ c515c531-dd2f-4f73-be84-7e9b73fbb74e
md"""
Derive the unit space for the atmosphere,
- The idea is to have a ray moving in an inhomogeneous media

1. Let move from the real space to the normalized space
...
\end{bmatrix}
```
2. Let write the radius as a function of the distance from the unit circle

```math
\begin{matrix}
h_a=h/a \\
f_l(h)=\left(1-e^2_l(h)\right)=1-\left(\sqrt{1-\frac{b+h}{a+h}}\right)^2 =\\
=\sqrt{\frac{b^2+2bh+h^2}{a^2+2ah+h^2}}=\\
=\sqrt{\frac{(1-e^2)+2\sqrt{1-e^2}\sqrt{1-e_h^2}+(1-e_h^2)}{1+2\sqrt{1-e_h^2}+(1-e_h^2)}}\\
f\rightarrow \sqrt{1-e^2}\qquad
f_h\rightarrow \sqrt{1-e_h^2}
\\
f_l(h)=\sqrt{\frac{(f+f_h)^2}{(1+f_h)^2}}=\sqrt{(1-e^2)}
\sqrt{\frac{(1+h/b)^2}
{(1+h/b*\sqrt{(1-e^2)})^2}}=\\
=f*\sqrt{\frac{(1+h/b)^2}{(1+h/b*f)}}\\
h_b=h/b=h_a\sqrt{1-e_h^2(h)} \\
\end{matrix}
```
```math
f_l(e)=f*\sqrt{\frac{(1+h/b)^2}{(1+h/b*f)}}
```

"""

# ╔═╡ ef83208b-e732-4eeb-b5b2-26db56247f28
let
	@variables ∂x∂r_ ∂x∂θ_ ∂y∂r_ ∂y∂θ_ ∂r∂h_ ∂r∂θ_  ∂θ∂h_ Δθ Δh

	A1=[∂x∂r_ ∂x∂θ_; ∂y∂r_ ∂y∂θ_]
	B1=[∂r∂h_ ∂r∂θ_;  ∂θ∂h_  1]
	C1=[Δh 0;0 Δθ]
...
manifest_format = "2.0"
project_hash = "bd9fd9a986b312c395295263d01bf2c0ec0d6429"
